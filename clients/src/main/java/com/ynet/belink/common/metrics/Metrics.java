...
public class Metrics implements Closeable {
    ...
    private final ConcurrentMap<MetricName, BelinkMetric> metrics;
  ...
  /**
    ...
    public Metrics(MetricConfig defaultConfig, List<MetricsReporter> reporters, Time time, boolean enableExpiration) {
        ...
        for (MetricsReporter reporter : reporters)
            reporter.init(new ArrayList<BelinkMetric>());
...
//                }
            ...
            synchronized (sensor) {
                synchronized (this) {
                    if (sensors.remove(name, sensor)) {
                        for (BelinkMetric metric : sensor.metrics())
    ...
    public synchronized void addMetric(MetricName metricName, MetricConfig config, Measurable measurable) {
        BelinkMetric m = new BelinkMetric(new Object(),
     ...
     *
     * @param metricName The name of the metric
     * @return the removed `BelinkMetric` or null if no such metric exists
     */
    public synchronized BelinkMetric removeMetric(MetricName metricName) {
        BelinkMetric metric = this.metrics.remove(metricName);
        if (metric != null) {
            for (MetricsReporter reporter : reporters)
    ...
    synchronized void registerMetric(BelinkMetric metric) {
     ...
     */
    public Map<MetricName, BelinkMetric> metrics() {
        return this.metrics;
    }
    ...
    public BelinkMetric metric(MetricName metricName) {
