...
object CompressionCodec {
  def getCompressionCodec(codec: Int): CompressionCodec = {
    codec match {
      ...
      case _ => throw new belink.common.UnknownCodecException("%d is an unknown compression codec".format(codec))
    }
  }
  def getCompressionCodec(name: String): CompressionCodec = {
    name.toLowerCase(Locale.ROOT) match {
      ...
      case _ => throw new belink.common.UnknownCodecException("%s is an unknown compression codec".format(name))
    }
  }
}
