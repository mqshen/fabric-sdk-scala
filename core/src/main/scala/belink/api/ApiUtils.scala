...
object ApiUtils {
  ...
  def writeShortString(buffer: ByteBuffer, string: String) {
    if(string == null) {
      ...
      if(encodedString.length > Short.MaxValue) {
        throw new BelinkException("String exceeds the maximum size of " + Short.MaxValue + ".")
      } else {
   ...
   * range. If not, throw BelinkException.
   */
  def readIntInRange(buffer: ByteBuffer, name: String, range: (Int, Int)): Int = {
    val value = buffer.getInt
    if(value < range._1 || value > range._2)
      throw new BelinkException(name + " has value " + value + " which is not in the range " + range + ".")
    else value
  }
