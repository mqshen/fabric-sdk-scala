...
object Acl {
  val WildCardPrincipal: BelinkPrincipal = new BelinkPrincipal(BelinkPrincipal.USER_TYPE, "*")
  ...
  def fromJson(aclJson: String): Set[Acl] = {
    ...
    Json.parseFull(aclJson) match {
      case Some(m) =>
        ...
        aclSet.foreach(item => {
          val principal: BelinkPrincipal = BelinkPrincipal.fromString(item(PrincipalKey).asInstanceOf[String])
 ...
 */
case class Acl(principal: BelinkPrincipal, permissionType: PermissionType, host: String, operation: Operation) {
