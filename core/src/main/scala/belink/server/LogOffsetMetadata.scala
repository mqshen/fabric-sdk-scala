...
case class LogOffsetMetadata(messageOffset: Long,
  ...
  def onOlderSegment(that: LogOffsetMetadata): Boolean = {
    if (messageOffsetOnly())
      throw new BelinkException(s"$this cannot compare its segment info with $that since it only has message offset info")
  ...
  def positionDiff(that: LogOffsetMetadata): Int = {
    if(!onSameSegment(that))
      throw new BelinkException(s"$this cannot compare its segment position with $that since they are not on the same segment")
    if(messageOffsetOnly())
      throw new BelinkException(s"$this cannot compare its segment position with $that since it only has message offset info")
