...
class LogManager(val logDirs: Array[File],
  ...
  private def createAndValidateLogDirs(dirs: Seq[File]) {
    if(dirs.map(_.getCanonicalPath).toSet.size < dirs.size)
      throw new BelinkException("Duplicate log directory found: " + logDirs.mkString(", "))
    for(dir <- dirs) {
      if(!dir.exists) {
        info("Log directory '" + dir.getAbsolutePath + "' not found, creating it.")
        val created = dir.mkdirs()
        if(!created)
          throw new BelinkException("Failed to create data directory " + dir.getAbsolutePath)
      }
      if(!dir.isDirectory || !dir.canRead)
        throw new BelinkException(dir.getAbsolutePath + " is not a readable log directory.")
    }
  }
  ...
  private def lockLogDirs(dirs: Seq[File]): Seq[FileLock] = {
    dirs.map { dir =>
      ...
      if(!lock.tryLock())
        throw new BelinkException("Failed to acquire lock on file .lock in " + lock.file.getParentFile.getAbsolutePath + 
  ...
  def asyncDelete(topicPartition: TopicPartition) = {
    ...
    if (removedLog != null) {
      ...
      if (renameSuccessful) {
        ...
        throw new BelinkStorageException("Failed to rename log directory from " + removedLog.dir.getAbsolutePath + " to " + renamedDir.getAbsolutePath)
      }
    }
  }
...
object LogManager {
  def apply(config: BelinkConfig,
    ...
    new LogManager(logDirs = config.logDirs.map(new File(_)).toArray,
      topicConfigs = Map("topic" -> topicConfigs),
