          ...
          override def next(): (AbortedTxn, Int) = {
            try {
              ...
              if (abortedTxn.version > AbortedTxn.CurrentVersion)
                throw new BelinkException(s"Unexpected aborted transaction version ${abortedTxn.version}, " +
              ...
              case e: IOException =>
                ...
                throw new BelinkException(s"Failed to read from the transaction index $file", e)
            }
          }
        }
    }
