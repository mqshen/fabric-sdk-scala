...
import belink.api.KAFKA_0_11_0_IV2
import belink.common._
import belink.metrics.BelinkMetricsGroup
import belink.server.{BrokerTopicStats, FetchDataInfo, LogOffsetMetadata}
import belink.utils._
import com.ynet.belink.common.errors.{CorruptRecordException, OffsetOutOfRangeException, RecordBatchTooLargeException, RecordTooLargeException, UnsupportedForMessageFormatException}
import com.ynet.belink.common.record._
import com.ynet.belink.common.requests.{IsolationLevel, ListOffsetRequest}
...
import belink.message.{BrokerCompressionCodec, CompressionCodec, NoCompressionCodec}
...
class Log(@volatile var dir: File,
          ...
          val producerStateManager: ProducerStateManager) extends Logging with BelinkMetricsGroup {

  import belink.log.Log._
...
//    tags)
   ...
   * @throws BelinkStorageException If the append fails due to an I/O error.
  ...
  private def append(records: MemoryRecords, isFromClient: Boolean, assignOffsets: Boolean, leaderEpoch: Int): LogAppendInfo = {
    ...
    try {
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          ...
          val validateAndOffsetAssignResult = try {
            ...
            case e: IOException => throw new BelinkException("Error in validating messages while appending to log '%s'".format(name), e)
          }
      ...
      case e: IOException => throw new BelinkStorageException("I/O exception in append to log '%s'".format(name), e)
    }
  }
  ...
  def fetchOffsetsByTimestamp(targetTimestamp: Long): Option[TimestampOffset] = {
    ...
    if (config.messageFormatVersion < KAFKA_0_11_0_IV2 &&
  ...
  def roll(expectedNextOffset: Long = 0): LogSegment = {
    ...
    lock synchronized {
      ...
      if(prev != null)
        throw new BelinkException("Trying to roll a new log segment for topic partition %s with start offset %d while it already exists.".format(name, newOffset))
   ...
   *
   * @throws BelinkStorageException if the file can't be renamed and still exists
   */
...
object Log {
  ...
  def parseTopicPartitionName(dir: File): TopicPartition = {
    if (dir == null)
      throw new BelinkException("dir should not be null")

    def exception(dir: File): BelinkException = {
      new BelinkException(s"Found directory ${dir.getCanonicalPath}, '${dir.getName}' is not in the form of " +
