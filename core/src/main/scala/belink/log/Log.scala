...
import belink.common.{BelinkStorageException, LongRef}
...
class Log(@volatile var dir: File,
          ...
          val pidSnapshotCreationIntervalMs: Int = 60 * 1000) extends Logging {
          ...
          if (validateAndOffsetAssignResult.messageSizeMaybeChanged) {
            for (batch <- validRecords.batches.asScala) {
              if (batch.sizeInBytes > config.maxMessageSize) {
                ...
                //BrokerTopicStats.getBrokerTopicStats(Topic.topic).bytesRejectedRate.mark(records.sizeInBytes)
