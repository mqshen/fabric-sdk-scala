...
import belink.server.epoch.LeaderEpochCache
import belink.server.{FetchDataInfo, LogOffsetMetadata}
import belink.utils._
import com.ynet.belink.common.errors.CorruptRecordException
import com.ynet.belink.common.record.FileRecords.LogOffsetPosition
...
class LogSegment(val log: FileRecords,
  ...
  def changeFileSuffixes(oldSuffix: String, newSuffix: String) {
    ...
    def BelinkStorageException(fileType: String, e: IOException) =
      new BelinkStorageException(s"Failed to change the $fileType file suffix from $oldSuffix to $newSuffix for log segment $baseOffset", e)

    try log.renameTo(new File(CoreUtils.replaceSuffix(log.file.getPath, oldSuffix, newSuffix)))
    catch {
      case e: IOException => throw BelinkStorageException("log", e)
    }
    try index.renameTo(new File(CoreUtils.replaceSuffix(index.file.getPath, oldSuffix, newSuffix)))
    catch {
      case e: IOException => throw BelinkStorageException("index", e)
    }
    try timeIndex.renameTo(new File(CoreUtils.replaceSuffix(timeIndex.file.getPath, oldSuffix, newSuffix)))
    catch {
      case e: IOException => throw BelinkStorageException("timeindex", e)
    }
    try txnIndex.renameTo(new File(CoreUtils.replaceSuffix(txnIndex.file.getPath, oldSuffix, newSuffix)))
    catch {
      case e: IOException => throw BelinkStorageException("txnindex", e)
    }
  }
   ...
   *
   * @throws BelinkStorageException if the delete fails.
   */
  def delete() {
    ...
    if (!deletedLog && log.file.exists)
      throw new BelinkStorageException("Delete of log " + log.file.getName + " failed.")
    if (!deletedIndex && index.file.exists)
      throw new BelinkStorageException("Delete of index " + index.file.getName + " failed.")
    if (!deletedTimeIndex && timeIndex.file.exists)
      throw new BelinkStorageException("Delete of time index " + timeIndex.file.getName + " failed.")
    if (!deletedTxnIndex && txnIndex.file.exists)
      throw new BelinkStorageException("Delete of transaction index " + txnIndex.file.getName + " failed.")
  }
...
object LogFlushStats extends BelinkMetricsGroup {
  val logFlushTimer = new BelinkTimer(newTimer("LogFlushRateAndTimeMs", TimeUnit.MILLISECONDS, TimeUnit.SECONDS))
}
