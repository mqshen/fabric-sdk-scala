...
import belink.log.Log
import belink.server.{LogOffsetMetadata, LogReadResult}
import belink.utils.Logging
import com.ynet.belink.common.BelinkException
import com.ynet.belink.common.errors.OffsetOutOfRangeException
...
class Replica(val brokerId: Int,
  ...
  private def logEndOffset_=(newLogEndOffset: LogOffsetMetadata) {
    if (isLocal) {
      throw new BelinkException(s"Should not set log end offset on partition $topicPartition's local replica $brokerId")
    } else {
  ...
  def maybeIncrementLogStartOffset(offset: Long) {
    if (isLocal) {
      ...
      throw new BelinkException(s"Should not try to delete records on partition $topicPartition's non-local replica $brokerId")
    }
  }

  private def logStartOffset_=(newLogStartOffset: Long) {
    if (isLocal) {
      throw new BelinkException(s"Should not set log start offset on partition $topicPartition's local replica $brokerId " +
  ...
  def highWatermark_=(newHighWatermark: LogOffsetMetadata) {
    if (isLocal) {
      ...
      throw new BelinkException(s"Should not set high watermark on partition $topicPartition's non-local replica $brokerId")
    }
  }
  ...
  def convertHWToLocalOffsetMetadata() = {
    if (isLocal) {
      ...
      throw new BelinkException(s"Should not construct complete high watermark on partition $topicPartition's non-local replica $brokerId")
    }
  }
