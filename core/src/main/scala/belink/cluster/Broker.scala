...
import belink.common.{BelinkException, BrokerEndPointNotAvailableException, BrokerNotAvailableException}
import belink.utils.Json
    ...
    try {
      Json.parseFull(brokerInfoString) match {
        case Some(m) =>
          ...
          val endpoints =
            if (version < 1)
              throw new BelinkException(s"Unsupported version of broker registration: $brokerInfoString")
            else if (version == 1) {
      ...
      case t: Throwable =>
        throw new BelinkException(s"Failed to parse the broker info from zookeeper: $brokerInfoString", t)
    }
  }
