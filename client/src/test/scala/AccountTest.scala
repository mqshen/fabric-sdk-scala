...
/**
  ...
  */
object AccountTest {
  val CHAIN_CODE_NAME = "account"
  val CHAIN_CODE_PATH = "github.com/chaincode/account/"
  val CHAIN_CODE_VERSION = "1"
  val accountType = "test5"

  val testDetail = "[{\"title\": \"还信用卡\", \"balance\": 100000, \"amount\": 200000, \"time\": \"2017-05-01 13:21:00\"} , {\"title\": \"转账\", \"balance\": 200000, \"amount\": 100000, \"time\": \"2017-05-01 13:21:00\"}]"
  ...
  def main(args: Array[String]): Unit = {
    ...
    def doIssue(): Unit = {
      ...
      val invokeProposalRequest = new InvokeProposalRequest(CHAIN_CODE_PATH, CHAIN_CODE_NAME,
        chainCodeID, "issue", Seq(accountType, assetAddress, "5000"))
    ...
    def doAssign(): Unit = {
      ...
      val assignProposalRequest = new InvokeProposalRequest(CHAIN_CODE_PATH, CHAIN_CODE_NAME,
        chainCodeID, "assign", Seq(accountType, ownerAddress, testDetail))
    ...
    def doQuery(): Unit = {
      ...
      val userQueryProposalRequest = new QueryProposalRequest(CHAIN_CODE_PATH, CHAIN_CODE_NAME,
        chainCodeID, "query", Seq(ownerAddress, accountType))
    ...
    def doQueryOrg(): Unit = {
      ...
      val userQueryProposalRequest = new QueryProposalRequest(CHAIN_CODE_PATH, CHAIN_CODE_NAME,
        chainCodeID, "queryOrg", Seq(accountType))
      chain.sendQueryProposal(userQueryProposalRequest, admin).map { res =>
        val successful = res.filter(x => x.isVerified && x.status == ChainCodeResponse.SUCCESS)
        val failed = res.filter(x => !x.isVerified || x.status != ChainCodeResponse.SUCCESS)
        if (failed.size > 0)
    ...
    def doTest() = {
      val detailTransferProposalRequest = new InvokeProposalRequest(CHAIN_CODE_PATH, CHAIN_CODE_NAME,
        chainCodeID, "transferWithDetail", Seq(accountType, "bubiV8hUtmXcaBRgVgZGHDnCt9b85K4bRkf9bYFc", "[{\"expire\":2000000000,\"amount\":2}]"))
